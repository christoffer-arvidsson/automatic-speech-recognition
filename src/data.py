...
def apply_preemphasis_filter(signal):
    ...
    pre_emphasis = 0.97
    emph_signal = tf.concat(
        [signal[:1], signal[:1] - pre_emphasis * signal[:-1]], 0)
...
def apply_log_mel_spec(signal):
    w = tf.signal.linear_to_mel_weight_matrix(num_mel_bins=100,
...
def preprocess_dataset(dataset):
    ...
    new = (dataset.map(map_func=lambda x, y: preprocess_pipeline(x, y),
